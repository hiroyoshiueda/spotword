		...
		if(typeof options == 'string')
		{
			//check if booklet has been initialized
			 if($(this).data('booklet')){
				...
				else if(command == 'clearpollhash'){ obj.clearPollHash() }
			 }
		}
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	var self, opts, b, src,
		...
		overlays, overlayN, overlayP, overlayC, tabs, tabN, tabP, arrows, arrowN, arrowP, next, prev, ctrlsN, ctrlsP, firstBtn, lastBtn, hashTimer,
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	$.extend(self,
	{
		...
		last : function(){
			if(!busy){
				var targetNum = (opts.pTotal % 2 != 0) ? opts.pTotal - 1 : opts.pTotal;
				self.gotoPage(opts.pTotal - 4);
			}
		},
		clearPollHash : function(){
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	if(opts.first){
		firstBtn = $(opts.first);
		firstBtn.click(function(e){e.preventDefault(); self.first();});
	}
	if(opts.last){
		lastBtn = $(opts.last);
		lastBtn.click(function(e){e.preventDefault(); self.last();});
	}

	//add overlays
	if(opts.overlays){
		overlayP = $('<div class="b-overlay b-overlay-prev b-prev" title="'+SR_PREV+'"><div id="b-overlay-prev-m"></div></div>').appendTo(b);
		overlayN = $('<div class="b-overlay b-overlay-next b-next" title="'+SR_NEXT+'"><div id="b-overlay-next-m"></div></div>').appendTo(b);
		overlayC = $('<div id="b-overlay-center" title="'+SR_NEW_READ+'"></div>').appendTo(b);
...
//		if($.support.opacity){
//			overlayP.find('#b-overlay-prev-m').click(function(e){
//				e.preventDefault();
//				self.prev();
//			}).hover(
//				function(){$(this).stop().fadeTo('fast', 0.8);},
//				function(){$(this).stop().fadeTo('fast', 0.3);}
//			);
//			overlayN.find('#b-overlay-next-m').click(function(e){
//				e.preventDefault();
//				self.next();
//			}).hover(
//				function(){$(this).stop().fadeTo('fast', 0.8);},
...
//		}
		...
		overlayC.click(opts.overlayopen).hover(
			function(){$(this).stop().fadeTo('fast', 0.8);},
	...
	//add arrows
	if(opts.arrows){
		arrowP = $('<div class="b-arrow b-arrow-prev b-prev" title="'+SR_PREV+'"><div>'+SR_PREV+'</div></div>').appendTo(b);
		arrowN = $('<div class="b-arrow b-arrow-next b-next" title="'+SR_NEXT+'"><div>'+SR_NEXT+'</div></div>').appendTo(b);
		arrows = b.find('.b-arrow');

		//update ctrls for RTL direction
		if(opts.direction == 'RTL'){
			arrowN.html('<div>'+SR_PREV+'</div>').attr('title',SR_PREV);
			arrowP.html('<div>'+SR_NEXT+'</div>').attr('title',SR_NEXT);
		}
	}
	...
	if(opts.arrows){
		if($.support.opacity){
			ctrlsN.hover(
				function(){arrowN.stop().fadeTo('fast', 1);},
				function(){arrowN.stop().fadeTo('fast', 0.5);
			});
			ctrlsP.hover(
				function(){arrowP.stop().fadeTo('fast', 1);},
				function(){arrowP.stop().fadeTo('fast', 0.5);
			});
		}else{
//			ctrlsN.hover(
//				function(){arrowN.show();},
//				function(){arrowN.hide();
//			});
//			ctrlsP.hover(
//				function(){arrowP.show();},
//				function(){arrowP.hide();
//			});
		}
//		if($.support.opacity){
//			ctrlsN.hover(
//				function(){arrowN.find('div').stop().fadeTo('fast', 1);arrowN.stop().fadeTo('fast', 1);},
//				function(){arrowN.find('div').stop().fadeTo('fast', 0.5);arrowN.stop().fadeTo('fast', 0.5);
//			});
//			ctrlsP.hover(
//				function(){arrowP.find('div').stop().fadeTo('fast', 1);arrowP.stop().fadeTo('fast', 1);},
//				function(){arrowP.find('div').stop().fadeTo('fast', 0.5);arrowP.stop().fadeTo('fast', 0.5);
//			});
//		}else{
//			ctrlsN.hover(
...
//		}
	...
	if(opts.hash){
		setupHash();
		clearInterval(hashTimer);
		hashTimer = setInterval(function(){pollHash()}, 250);
	}
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	function resetPages(){
		...
		wraps  = b.find('.b-wrap');
		pcover = b.find('.b-page-cover');

		//update css
		wraps.attr('style','');
		wraps.css({'width':opts.pWidth-(opts.pagePadding*2), 'height':opts.pHeight-(opts.pagePadding*2)});
		pcover.css({'width':opts.pWidth, 'height':opts.pHeight});
...
//		p2.css({'left':opts.pWidth, 'width':opts.pWidth, 'opacity':1, 'height':opts.pHeight});
		p2.css({'left':opts.pWidth, 'width':opts.pWidth, 'height':opts.pHeight});
...
//		}
	...
	function initAnim(diff, inc, shadow){
		...
		if(inc && diff > 2){
			...
			b.find('.b-page-'+opts.curr+' .b-wrap').show().css({'width':opts.pWidth-(opts.pagePadding*2), 'height':opts.pHeight-(opts.pagePadding*2)});
			b.find('.b-page-'+(opts.curr+1)+' .b-wrap').show().css({'width':opts.pWidth-(opts.pagePadding*2), 'height':opts.pHeight-(opts.pagePadding*2)});
