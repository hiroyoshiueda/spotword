...
function doOpenReader()
{
	var url = '{/literal}/reader/{$form.id}/{literal}';
	openwin(url, 'spotreader');
}
//]]>
</script>
{/literal}
...
<div id="book-reader">
<div id="epub-reader"><div id="loading"><img src="/js/spotreader/images/loading.gif" /><div>読み込み中...</div></div></div>
</div>
...
<div class="column_wrapper{$form.errors.book_comment|@errorclass}">
	...
	<div id="book-comment-mask"><div>コメントを投稿するには<a href="#" onclick="jump('/login?loc={$base_url}&_hash=comment');return false;">ログイン</a>してください。</div></div>
...
<div id="book-contents-{$d.page_order}"{if $d.page_order!=$form.chapter-1} style="display:none;"{/if}>
...
<ul>
	<li style="padding-right:4px;">
		<a href="http://b.hatena.ne.jp/entry/{$smarty.const.app_site_url}book/{$form.id}/" class="hatena-bookmark-button" data-hatena-bookmark-title="{$title}" data-hatena-bookmark-layout="standard" title="このエントリーをはてなブックマークに追加"><img src="http://b.st-hatena.com/images/entry-button/button-only.gif" alt="このエントリーをはてなブックマークに追加" width="20" height="20" style="border: none;" /></a>
	</li>
	<li style="padding-right:4px;">
		<a href="http://twitter.com/share" class="twitter-share-button" data-url="{$smarty.const.app_site_url}book/{$form.id}/" data-count="none" data-via="spotwordjp" data-lang="ja">Tweet</a>
	</li>
	<li>
		<iframe src="http://www.facebook.com/plugins/like.php?href={$smarty.const.app_site_url|escape:'url'}{"book/`$form.id`/"|escape:'url'}&amp;layout=button_count&amp;show_faces=true&amp;width=80&amp;action=like&amp;colorscheme=light&amp;height=21" scrolling="no" frameborder="0" style="border:none; overflow:hidden; width:80px; height:21px;" allowTransparency="true"></iframe>
	</li>
	<li>
		<iframe scrolling="no" frameborder="0" allowTransparency="true" style="overflow:hidden; border:0; width:60px; height:20px" src="http://plugins.mixi.jp/favorite.pl?href={$smarty.const.app_site_url|escape:'url'}{"book/`$form.id`/"|escape:'url'}&service_key=483d62db980d90653000b22e6f55689539641240&show_faces=true&width=60"></iframe>
	</li>
{*	<li>
...
//	var loadingObj = $('<div id="loading"><div>読み込み中...</div></div>').prepend(loading);
//	$('#epub-reader').append(loadingObj);
	$('#epub-reader').spotreader({'load':'#book-contents','cover':'{/literal}{if $form.book.cover_path!=""}{$form.cover_path}/{$form.book.cover_path}{/if}{literal}','mode':'inline','width':630,'overlayopen':function(){doOpenReader();}});
